...
var APP = APP || {
    ...
    // 判断环形棋盘上从 start 顺时针走到 end 是否经过 target（不含起点，含终点）
    isSquarePassed: function(start, end, target, boardSize) {
        if (start === end) {
            return false;
        }
        if (end > start) {
            // 没绕圈：target 在 (start, end] 区间内
            return start < target && target <= end;
        } else {
            // 绕圈：target 在 (start, boardSize-1] 或 [0, end]
    ...
    movePlayer: function(dieCount) {
        ...
        // If pass paycheck get payday - 逐个独立判断，跨过几个发几次
        var paydaySquares = [5, 13, 21];
        for (var i = 0; i < paydaySquares.length; i++) {
            if (APP.isSquarePassed(previousPosition, currentPosition, paydaySquares[i], 24)) {
...
APP.finance = {
    statement: function() {
        ...
        if (player.fastTrack == true) {
            ...
            $("#win-pay-header").show();
            $("#expense-bar-title").text("INCREASE CASHFLOW DAY INCOME TO WIN");
            $("#passive-income-label").text("CASHFLOW DAY INCOME:");
            ...
            $("#win-pay-header").hide();
            $("#expense-bar-title").text("INCREASE PASSIVE INCOME TO ESCAPE THE RAT RACE");
            $("#passive-income-label").text("PASSIVE INCOME:");
            //show total expenses
    ...
    progressBar: function() {
        ...
        if (player.fastTrack == true) {
            // 官方：快车道进度 = 当前现金流日收入 / 目标（入圈基数 + 50000），与胜利判定同口径
            width = 100 * (player.cashFlowDay / player.winPay);
    ...
    getIncome: function(currentPlayer) {
        ...
        if (player.fastTrack == false) {
            ...
            // 官方：快车道总收入 = 现金流日收入（买企业时 cashFlowDay 已计入该企业现金流，不再重复累加）
            player.totalIncome = player.cashFlowDay;
        }
    ...
    getPayday: function(currentPlayer) {
        ...
        if (player.fastTrack == false) {
            ...
            // 官方：快车道 PAYDAY = 现金流日收入（cashFlowDay 已含全部快车道企业现金流，且快车道无支出）
