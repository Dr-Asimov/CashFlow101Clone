var FASTTRACK = {
    init: function() {
        ...
        // 官方（101）：起始现金流日收入 = 被动收入（四舍五入到最近千元）× 100
        playerObj.cashFlowDay = Math.round(playerObj.passiveIncome / 1000) * 1000 * 100;
        // 官方 FAQ：出圈时立即收取一次起始现金流日收入，此后每次经过 CASHFLOW DAY 格再收
        playerObj.cash += playerObj.cashFlowDay;
    ...
    movePlayer: function(dieCount) {
        ...
        var ftPaydaySquares = [10, 18, 30, 38];
        for (var i = 0; i < ftPaydaySquares.length; i++) {
            if (APP.isSquarePassed(previousPosition, currentPosition, ftPaydaySquares[i], 40)) {
	...
	winGame: function() {
		...
		document.getElementById("win-income-amount").innerHTML = APP.display.numWithCommas(Math.round(player.cashFlowDay));
